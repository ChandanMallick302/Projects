...
<body>
	...
	<div class="container text-center">
		...
		<p>
			<span><strong>Showing available flights from <%=source%>
					to <%=destination%><br> Date of travel: <%=travelDate%> <br>
					...
					<tr style="background-color: #F5F5F5">
						...
						<td>
							<form action="payment.jsp" method="post">
								<input type="hidden" id="flight-name" name="flight-name"
									value="${fl.getFlightName()}"> <input type="hidden"
									id="source" name="source" value="${fl.getSource()}"> <input
									type="hidden" id="destination" name="destination"
									value="${fl.getDestination()}"> <input type="hidden"
									id="ticket-price" name="ticket-price"
									value="${fl.getTicketPrice()}"> <input type="hidden"
									id="c-name" name="c-name" value="<%=cName%>"> <input
									type="hidden" id="c-email" name="c-email" value="<%=cEmail%>">
								<input type="hidden" id="c-mob" name="c-mob"
									value="<%=cMobile%>"> <input type="hidden"
									id="c-passport" name="c-passport" value="<%=cPassport%>">
								<input type="hidden" id="no-of-person" name="no-of-person"
									value="<%=noOfPerson%>"> <input type="hidden"
									id="travel-date" name="travel-date" value="<%=travelDate%>">
